...
void dust(int in_val, float small_amp) {
  ...
  for (int i = 0; i<width-1; i+=2) {
    for (int j = 0; j<height-1; j+=2) {
      ...
      //HSB:
      background_buffer.fill(random(0.76, 1), inHsb*small_amp);
      background_buffer.rect(i, j, 2, 2);
    }
  }
 ...
 // large dust
  for (int i = 0; i<30; i++) {
    ...
    background_buffer.fill(random(0.12, 0.66), random(inHsb*1.5, inHsb*4));
    background_buffer.rect(random(0, width-2), random(0, height-2), random(1, 3), random(1, 3));
  }
}
